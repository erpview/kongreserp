...
(() => {
  ...
  class ThreeDStage extends HTMLElement {
    constructor() {
      ...
      note.className = 'note';
      note.textContent = 'Obrót — przeciągnij · przybliżenie — kółko · przesunięcie — prawy przycisk';
      root.appendChild(note);
      this._toolbar = document.createElement('div');
      this._toolbar.className = 'toolbar';
      this._objBtn = document.createElement('button');
      this._objBtn.type = 'button';
      this._objBtn.textContent = 'Pobierz model OBJ';
      ...
      this._glbBtn.textContent = 'Pobierz model GLB';
    ...
    /** Podpisy interfejsu sceny — strona podaje je w swoim języku. */
    setLabels({ note, obj, glb } = {}) {
      const napis = this.shadowRoot && this.shadowRoot.querySelector('.note');
      if (napis && note) napis.textContent = note;
      if (this._objBtn && obj) this._objBtn.textContent = obj;
      if (this._glbBtn && glb) this._glbBtn.textContent = glb;
    }

    /** Kamera, sterowanie i płótno sceny — potrzebne stronie, która chce
     *  łapać obiekty wskaźnikiem (przestawianie mebli na stoisku). */
    get camera() { return this._camera; }
    get controls() { return this._controls; }
    get canvas() { return this._renderer && this._renderer.domElement; }
